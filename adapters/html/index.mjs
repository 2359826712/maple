...
function itemsFromPage(result, source) {
  ...
  const externalIdPattern = source.adapter_config.external_id_pattern
    ? new RegExp(source.adapter_config.external_id_pattern, 'i')
    : null;
  ...
  for (const link of document.querySelectorAll(selector)) {
    ...
    const externalIdMatch = externalIdPattern?.exec(url);
    items.push({
      url,
      externalId: link.getAttribute('data-id') || externalIdMatch?.[1] || null,
      ...
      metadata: {
        discovery_url: result.finalUrl,
        body_selector: source.adapter_config.body_selector || null,
      },
    });
  }
