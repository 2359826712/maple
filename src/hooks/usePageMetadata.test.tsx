...
describe('usePageMetadata', () => {
  ...
  it('updates discoverability and social metadata for the current page', () => {
    window.history.replaceState({}, '', '/guides/zh');
    render(<MetadataHarness />);

    expect(document.title).toBe('Lotus Boss Guide · MPStorys');
    expect(document.querySelector('link[rel="canonical"]')?.getAttribute('href')).toBe('https://mpstorys.com/guides/zh');
    ...
    expect(document.querySelector('link[hreflang="en"]')?.getAttribute('href')).toBe('https://mpstorys.com/guides/en');
    expect(document.querySelector('link[hreflang="zh-Hant"]')?.getAttribute('href')).toBe('https://mpstorys.com/guides/zh-hant');
    expect(document.querySelector('link[hreflang="x-default"]')?.getAttribute('href')).toBe('https://mpstorys.com/guides/en');
  });

  it('marks private pages as noindex and nofollow', () => {
    window.history.replaceState({}, '', '/account/ja');
